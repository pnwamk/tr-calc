...
\and
  ...
  o_{r} = (\refpe{x}(y)[o_2 / x])[o_1 / y] \: where y \notin o_2
  {\TypeOf{\Gamma}{(ref \: e_1 \: e_2)}{\tau}{\NOT{\Ftype_{x}}[o_r / x]}{o_r}}
...
\and
  \inferrule[L-Ineq]
  {\Gamma \vdash \Ntype_{\pi(x)} \\ \Gamma \vdash \Phi' \\\\ 
    \textrm{unsatisfiable}(\Phi' \Join (\Phi[x / \diamond]))}
  {\Gamma \vdash \deptype{\Phi}_{\pi(x)}}

\and
  \inferrule[L-IneqSimple]
  {\Gamma, \Ntype_{\pi(x)}, \Phi[x / \diamond] \vdash \psi}
  {\Gamma, \deptype{\Phi}_{\pi(x)} \vdash \psi}
